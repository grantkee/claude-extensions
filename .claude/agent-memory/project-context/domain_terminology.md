...
**Wrapper agents:** Several agents (tn-write-e2e-agent, tn-write-proptest-agent, tn-write-docs-agent, tn-review-agent) exist solely to wrap a single skill with proper context loading and output verification. They are thin shells, not independent logic.
...
**Security eval is skill-driven:** `tn-security-eval` is a *skill* (not an agent) that orchestrates 10 named *agents* in parallel. This is unusual — most skills are passive prompts, but tn-security-eval actively spawns subagents.
