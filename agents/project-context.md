...
description: "Use this agent at the START of every plan mode session. MANDATORY — spawn this FIRST before any implementation or analysis agents. Do NOT skip.\n\nAnalyzes repository architecture and writes `.claude/project-context.md` — the shared context file all downstream subagents reference. Checks freshness automatically and returns early if context is current.\n\nWHEN to spawn (detect these proactively):\n- Entering plan mode for any task → spawn FIRST\n- Multiple subagents about to launch → spawn FIRST for shared context\n- `.claude/project-context.md` is missing or stale → spawn to regenerate\n- Multi-repo task → one instance per unique git remote, in parallel\n\nExamples:\n\n- Example 1:\n  Context: User requests a feature requiring plan mode.\n  assistant: \"Starting plan mode. Spawning project-context first to ensure shared context.\"\n  <spawns project-context agent as the very first action>\n\n- Example 2:\n  Context: Multi-repo task spanning two repositories.\n  assistant: \"Two repos involved. Spawning project-context for each in parallel before planning.\"\n  <spawns two project-context agents simultaneously>"
